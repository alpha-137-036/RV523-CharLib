...
def main():
    ...
    parser_characterize.add_argument('cellList', type=str,
            help='The json5 file specifying the list of cells to characterize')
    parser_characterize.add_argument('settings', type=str,
            help='The YAML file specifying common settings. Use the original CharLib syntax')
...
def run_charlib(args):
    ...
    with open(args.cellList) as f:
        cellList = json5.load(f)
    
    with open(args.settings) as f:
        config = yaml.safe_load(f)
    cells_config = {}
    config["cells"] = cells_config

    for cell in cellList:
        try:
            with open(f"{cell}/{cell}_config.json5", 'r') as f:
                print(f"[{cell}]")
                cell_config = json5.load(f)
                if not "functions" in cell_config:
                    print(f"Warning: functions not specified")
                else:
                    # remove pins
                    del cell_config["pins"]
                    # Calculate size
                    cell_config["area"] = cell_config["width"] # area unit is Transistor count
                    del cell_config["width"]
                    cells_config[cell] = cell_config
                    # Add pointer to netlist
                    cell_config["netlist"] = f"{cell}/{cell}.ckt"
